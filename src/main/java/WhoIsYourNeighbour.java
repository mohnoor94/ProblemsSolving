...
/**
 * Problem: Given a tree of nodes (that has 3 pointers: left, right, and next), you are required to set the value of the
 * `next` pointer of every node to it's neighbour.
 * The neighbour of any node is the node that comes to the right of that node at the same level, or null if there are
 * no nodes.
 * <p>
 * Example:
 * for the tree:
